...
export class DataTableComponent extends Component {
	...
	/**
	 * 
	 * @param {*} header Liste de string
	 * @param {*} content Liste de liste de string
	 * @param {*} callback Callbak takes the index of the element clicked on to enable some logic on this element
	 */
	constructor(header, content, onClickCallback = null) {
		super();
		...
		setTimeout(() => this.makeAllSortable(), 50);
	}
	...
	myFunctionFilter() {
		this.__html__.querySelector("#filter")?.addEventListener("keyup", () => {
	...
	click() {
		...
		for (let i = 0; i < elems.length; i++) {
			...
			elem?.addEventListener("click", () => {
				...
				// callback takes the index of the seleced argument as parameter to able some logic
				try {
	...
	updateContent(newContent) {
		...
		this.setHtmlContent();
		this.__refresh__();
		setTimeout(() => this.makeAllSortable(), 20);
	}
	...
	makeSortable(table) {
		...
		while (--i >= 0) (function (i) {
			var dir = 1;
			th[i].addEventListener('click', function () { self.sortTable(table, i, (dir = 1 - dir)) });
		}(i));
